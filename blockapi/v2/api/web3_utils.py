    ...
    if network == 'optimism':
        ...
        url = {
            'anyblock': (
                ...
                f'{network}/rpc/{api_key}/'
            ),
            'infura': f'https://{network}.infura.io/v3/{api_key}',
            'pokt': env_variables.get('POKT_NODE'),
            'local': env_variables.get('LOCAL_ETH_NODE'),
        }.get(provider_name)

    if not url:
        raise ValueError(
            f'Invalid provider name: {provider_name} for network {network}'
        )
