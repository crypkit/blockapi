...
class BlockApi:
    ...
    def get_balance_from_random_api(self, symbol, address):
        ...
        return self._call_method_from_random_api(symbol, address, 'get_balance')

    def _call_method_from_random_api(self, symbol, address, method):
        api_classes = self.get_shuffled_api_classes_for_coin(symbol)
        filtered_api_classes = self.filter_suitable_api_classes(api_classes, symbol, address)
    ...
    def get_shuffled_api_classes_for_coin(self, symbol):
        api_classes = self.get_api_classes_for_coin(symbol)
        random.shuffle(api_classes)
        return api_classes

    def get_api_classes_for_coin(self, symbol):
        return [i for i in self.get_active_api_classes() if
                i.symbol and i.symbol == symbol]

    def filter_suitable_api_classes(self, api_classes, symbol, address):
        address_info = self.get_address_info(symbol, address)
    ...
    def get_random_api_class_for_coin(self, symbol, exclude=None):
        api_classes = self.get_api_classes_for_coin(symbol)
    ...
    def get_all_supported_coins(self):
        return list(set(c.symbol for c in self.get_active_api_classes()
                        if c.symbol))

    def get_active_api_classes(self):
        inheritors = self._get_all_inheritors()
    ...
    def _get_all_inheritors(self):
        all_inheritors = []

        tridy = self._get_subclasses(blockapi.api)

        for trida in tridy:
            tridy_sub = self._get_subclasses(trida)
            for trida_sub in tridy_sub:
                if inspect.isclass(trida_sub) and \
                    ...
                    grandchildren = self._inheritors(trida_sub)
                    if len(grandchildren) > 0:
                        all_inheritors += self._inheritors(trida_sub)

        return all_inheritors

    def get_working_apis_for_coin(self, symbol, debug=False):
        coin_classes = self.get_api_classes_for_coin(symbol)
    ...
    def get_working_apis(self, debug=False):
        all_coins = self.get_all_supported_coins()
        ok_apis = {}

        for a_coin in all_coins:
            ok_apis[a_coin] = self.get_working_apis_for_coin(a_coin, debug=debug)

        return ok_apis

    def check_address_valid(self, symbol, address):
        return self.get_address_info(symbol, address).valid
